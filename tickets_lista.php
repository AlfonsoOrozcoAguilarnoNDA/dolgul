...
// Consolidación de tickets_abiertos.php (Chunk 17) y tickets_cerrados.php (Chunk 18)
// $tipo: 'abiertos' o 'cerrados'

function tickets_lista($tipo = 'abiertos'){
global $link;

$tipo = ($tipo === 'cerrados') ? 'cerrados' : 'abiertos';

$rol = $_SESSION['user_role'] ?? 'User';
$empresa_usuario = (int)($_SESSION['empresa_id'] ?? 1);
$user_id = (int)($_SESSION['user_id'] ?? 0);

// Texto/columnas dependientes del tipo de listado
if ($tipo === 'abiertos') {
    $where = "WHERE t.tickets_status IN ('Open', 'In Progress') AND t.tickets_child = 0 ";
    $order_by = "t.tickets_timestamp DESC";
    $titulo = "Tickets Abiertos / En Progreso";
    $mensaje_vacio = "No hay tickets abiertos en este momento.";
} else {
    $where = "WHERE t.tickets_status = 'Closed' AND t.tickets_child = 0 ";
    $order_by = "t.horacierre DESC";
    $titulo = "Tickets Cerrados";
    $mensaje_vacio = "No hay tickets cerrados que coincidan con tus permisos.";
}
...
// TODO: $label_procesos no está definido en ningún lado (bug heredado de los
// dos archivos originales). Definirlo aquí o quitar la referencia del <th>.
$label_procesos = $label_procesos ?? 'Proceso';
?>
...
<div class="table-responsive">
    <table class="table table-hover table-bordered">
            ...
            <tr class="cursor-pointer" onclick="window.location='ver_ticket.php?id=<?php echo $row['tickets_id']; ?>'">
                ...
                <td>
                    <?php if ($tipo === 'abiertos'): ?>
                        <?php $status_class = ($row['tickets_status'] === 'Open') ? 'badge-status-open' : 'badge-status-progress'; ?>
        ...
        <?php if (mysqli_num_rows($result) == 0): ?>
            <tr><td colspan="10" class="text-center py-4 text-muted"><?php echo htmlspecialchars($mensaje_vacio); ?></td></tr>
        <?php endif; ?>
        </tbody>
    </table>
</div>
